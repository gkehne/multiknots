...
# returns the name of the petal knot encoded by 'perm'
def pName(perm):
	s = str(dtNote(perm))
...
# returns the name of the uber knot encoded by 'uber'
def uName(uber):
	s = str(udtNote(uber))
	s = "DT:[(" + s[1:-1] + ")]"  # string trimming
	return snappy.Manifold(s).identify()
...
def pVol(perm):
	s = str(dtNote(perm))  # get s into snappy format
	s = "DT:[(" + s[1:-1] + ")]"  # string trimming
...
def uVol(uber):
	s = str(udtNote(uber))  # get s into snappy format
	s = "DT:[(" + s[1:-1] + ")]"  # string trimming
	return snappy.Manifold(s).volume()
...
def dtNote(perm):
	...
	for i in range(n):  # petal knot, so strands are traversed in order
		for j in range(n):
			...
			# For i= strand 0, k is OTF 1,3,5, ... n-4, n-2, 2,4,6, ... n-3, n-1
			k = (i + sequence[j]) % n
			if N[i][k] != 0:
			...
			if k != i:
				...
				N[i][k] = counter
				counter = counter + 1
				# if it's an odd under-crossing:
				if (N[i][k]) % 2 == 0 and perm[i] - perm[k] > 0:
	...
	for i in range(n):
		for j in range(n):
			if i != j and N[i][j] % 2 == 1:  # take odd crossing numbers
				...
				# insert their symmetric (even entry) pairs in that spot
				dt[(N[i][j] - 1) / 2] = N[j][i]
	return dt


# generates DT notation for an UBER KNOT, given its central permutation
# and 'tieup' information
def udtNote(uber):
	# preliminaries
	perm = uber[0]
	tieup = uber[1]
	...
	for i in range(1, 2 * n):  # this gives the order for strands to be chased
		strand.append(tieup[(strand[i - 1] + n) % (2 * n)])
	for i in range(n):  # current strand/traversal direction
		j = strand[i]
		# this section computes the order in which this jth strand crosses
		# other strands. Example: for strand j=0, forwards, n odd, order is
		# [1,3,5, ... n-4, n-2, 2,4,6, ... n-3, n-1]
		sequence = []
		for y in range(m):
			sequence.append((2 * y + 1) % m)
		# parity=1 if strand # is even, -1 if it's odd:
		parity = 2 * ((j % n) % 2) - 1
		for x in range(m):  # fix sequence to account for current strand index
			sequence[x] = (j % n + parity * sequence[x]) % m
		if j >= n:  # depending on j, traversal of the strand should go backwards
			sequence.reverse()
		if not odd:  # remove the 'ghotst' n+1st strand
			sequence.remove(n)
		for k in range(n):
			l = sequence[k] % n
			if N[j % n][l] != 0:
	...
	for i in range(n):
		for j in range(n):
			if N[i][j] % 2 == 1:
				# indexes starting at 0, though numbers in crossings start at 1
				dt[(N[i][j] - 1) / 2] = N[j][i]
...
def removeloops(code):
	...
	while eliminating:
		...
		for j in range(len(code)):
			if j >= len(code):
				continue
			if (abs(2 * j + 1 - abs(code[j])) == 1 or
						(j == 0 and abs(code[j]) == max([max(code), -min(code)]))):
				eliminating = True
				code.remove(code[j])
